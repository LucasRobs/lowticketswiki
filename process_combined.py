...
# Normalize new complaints to match old format
def normalize_complaint(c, gateway):
    return {
        'gateway': gateway,
        'url': c.get('url', ''),
        'list_title': c.get('title', ''),
        'raw_text': c.get('body', '') or c.get('title', '')
    }

# Convert new complaints
normalized_perfectpay = [normalize_complaint(c, 'PerfectPay') for c in perfectpay_new]
normalized_cakto = [normalize_complaint(c, 'Cakto') for c in cakto_new]

# Combine all complaints
all_complaints = old_complaints + normalized_perfectpay + normalized_cakto
...
def extract_product_info(complaint):
    ...
    product_patterns = [
        ...
        r'(?:Stalkeia|Stalkea|Stalker|Spygram|hqflix|Chat GPT|PLANO FREELANCER|Malu|Octuz|Cashcolor|Converza|Retrato da Alma|Sua Assistente)[\w\s\.\-]*',
        r'["\']([^"\']+)["\']',
    ]
    ...
    if any(kw in text_lower for kw in ['espion', 'monitor', 'spy', 'stalke', 'whatsapp', 'rastreador', 'acesso a perfis', 'direct', 'clonagem']):
    ...
    # IA/ChatGPT
    elif any(kw in text_lower for kw in ['chat gpt', 'chatgpt', 'ia ', 'inteligência artificial', 'gpt plus', 'infinity', 'octuz', 'malu']):
    ...
    # Consultoria/cursos
    elif any(kw in text_lower for kw in ['consultoria', 'curso', 'mentoria', 'accenture', 'setup', 'cashcolor']):
    ...
    elif any(kw in text_lower for kw in ['pagamento não recebido', 'não recebi', 'cobrança indevida', 'taxa', 'conta bloqueada']):
    ...
    if 'recorrência' in text_lower or 'mensal' in text_lower or 'anual' in text_lower or 'assinatura' in text_lower:
    ...
    termo_busca = produto
    for suffix in ['.ai', '.com', '.app', ' app', ' ai', ' pro', ' max', ' premium', ' - sua assistente']:
...
for termo, items in groups.items():
    ...
    tipo = "marca" if any(kw in termo.lower() for kw in ['stalkeia', 'spygram', 'hqflix', 'freelancer', 'chat gpt', 'infinity', 'accenture', 'malu', 'octuz', 'cashcolor', 'converza', 'retrato da alma', 'sua assistente', 'setup']) else "angulo"
    
    # Producer info (simplified)
    produtor_nome = "desconhecido"
    # Try to extract producer from text
    for item in items:
        desc = item.get('descricao', '').lower()
        if 'ks digital' in desc:
            produtor_nome = "KS Digital SA"
        elif 'converza' in desc:
            produtor_nome = "Converza.io"
        elif 'octuz' in desc:
            produtor_nome = "Octuz AI"
        elif 'cashcolor' in desc:
            produtor_nome = "Cashcolor"
        elif 'malu' in desc:
            produtor_nome = "Malu Assistente"
...
output = {
    ...
    "gateways": [
        {"nome": "PerfectPay", "slug": "perfectpay", "reclamacoes_ativas": 287224, "paginas_varridas": 5},
        {"nome": "Cakto", "slug": "cakto-pay", "reclamacoes_ativas": 0, "paginas_varridas": 5}
    ],
    "achados": achados
}
