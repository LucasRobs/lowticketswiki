...
def extract_product_info(complaint):
    ...
    # Cloaker / ferramentas de marketing
        ...
        nicho = "Ferramentas de marketing / Cloaker"
        angulos = ["facilidade_tecnologica", "autoridade"]
        sinais_cloaker.append("ferramenta_cloaker")
    
    # Reembolso/suporte genérico
    elif any(kw in text_lower for kw in ['reembolso', 'estorno', 'cancelamento', 'suporte', 'atendimento', 'não recebi', 'não entregue']):
        nicho = "Infoprodutos - suporte e reembolso"
        angulos = ["medo_perda_dinheiro", "confiança_quebrada"]
    
    # Pagamentos não recebidos
    elif any(kw in text_lower for kw in ['pagamento não recebido', 'não recebi', 'cobrança indevida', 'taxa', 'clonado', 'cartão']):
        nicho = "Serviços financeiros / Pix"
        angulos = ["medo_perda_dinheiro", "indignação"]
    
    # Conta digital produtor
    elif any(kw in text_lower for kw in ['conta digital', 'produtor', 'transferir saldo', 'validacao', 'saldo suficiente']):
    ...
    termo_busca = produto
    for suffix in ['.ai', '.com', '.app', ' app', ' ai', ' pro', ' max', ' premium', ' 2.0', ' 2', ' zero']:
...
for termo, items in groups.items():
    ...
    full_text = " ".join([item.get('descricao', '') for item in items])
    if 'ks digital' in full_text.lower():
        produtor_nome = "KS Digital SA"
    elif 'infinity' in full_text.lower():
        produtor_nome = "Infinity"
    elif 'converza' in full_text.lower():
...
for a in achados:
    print(f"  [{a['score_final']}] {a['produto']} ({a['gateway']}) - {a['acao_recomendada']} - Nicho: {a['nicho']}")