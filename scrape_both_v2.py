    ...
    async with async_playwright() as p:
        ...
        for page_num in range(1, max_pages + 1):
            ...
            try:
                ...
                # Wait for complaint cards to load - with shorter timeout
                try:
                    await page.wait_for_selector('[data-testid="complaint-listagem-v2-title-link"]', timeout=10000)
                except:
                    # If selector not found, check if page has any content
                    page_title = await page.title()
                    print(f"{gateway_name} Page {page_num}: Title = {page_title}")
                    ...
                    """)
                    if card_count == 0:
                        print(f"{gateway_name} Page {page_num}: No complaint cards found, stopping")
                        break
