...
async def scrape_gateway_pages(gateway_slug, gateway_name, max_pages=25):
        ...
        for page_num in range(1, max_pages + 1):
            ...
            try:
                ...
                complaints = await page.evaluate("""() => {
                    const items = document.querySelectorAll('article.ng-scope, article[data-testid="complaint-item"], .complaint-card, [class*="complaint"]');
                    const results = [];
                    for (const item of items) {
                        const link = item.querySelector('a[href*="/perfectpay/"], a[href*="/cakto-pay/"]');
                        if (link) {
                            const title = link.innerText.trim() || link.getAttribute('title') || '';
                            const href = link.href;
                            if (title && href && !href.includes('/empresa/')) {
...
async def main():
    perfectpay = await scrape_gateway_pages('perfectpay', 'PerfectPay', 25)
    ...
    cakto = await scrape_gateway_pages('cakto-pay', 'Cakto', 25)
